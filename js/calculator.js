$(document).ready(function() {
  ...
  function getStageText(stage_data) {
    ...
    if (stage_data["number"] >= 0) {
      ...
      result = "Nu se poate calcula pe baza datelor introduse";
    }

    return result;
  }

  function resetTnmData() {
    $("select[name=T]").val("");
    $("select[name=N]").val("");
    $("select[name=M]").val("");
  }
  ...
  function updateStageLabel(t, n, m) {
    ...
    if (t === "" || n === "" || m === "") {
      $("span[id=tnm]").html("Nu se poate calcula pe baza datelor introduse");
    } else {
      $("span[id=tnm]").html(t + n + m);
    }
  ...
  function resetClinicalData() {
    $("select[name=lymphs]").val("");
    $("select[name=metastases]").val("");
    $("select[name=ldh]").val("");
  ...
  function onBreslowChanged(object) {
    var breslow = object.target.value / 10.0;
  ...
  function onClinicalDataChanged() {
    ...
    var breslow = parseFloat($("input[id=breslow]").val() / 10.0);
